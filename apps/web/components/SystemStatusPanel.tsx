...
import type { RabbitQueueGroupStatus, SystemComponentStatus } from '@/lib/system-status'
import { useGetSystemStatusQuery } from '@/redux/api/systemStatusApi'
...
function QueueMetric({ label, value }: { label: string, value: number }) {
  return (
    <span className="rounded-[7px] border border-white/10 bg-white/5 px-2 py-1 text-[11px] leading-none text-muted-foreground">
...
function RabbitQueueRow({ queue }: { queue: RabbitQueueGroupStatus }) {
  ...
  return (
    <div className="rounded-[11px] border border-white/6 bg-white/1.5 px-3 py-2">
...
export function SystemStatusPanel() {
  const { data: status, error: queryError, isFetching, refetch } = useGetSystemStatusQuery(undefined, {
    pollingInterval: STATUS_REFRESH_INTERVAL_MS
  })

  const error = queryError ? 'Cannot load system status' : null

  const sortedComponents = useMemo(() => {
    const rank = { degraded: 1, down: 0, ok: 2 }
    return [...(status?.components ?? [])].sort(
    ...
    <section aria-label="Service status" className="nurt-panel p-[18px_20px]">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-px text-[11px] text-(--nurt-t3)">
        ...
        <div className="flex items-center gap-2">
          {status && (
            <span className={`rounded-[7px] border px-2.25 py-0.75 font-mono text-[10.5px] ${statusTone[status.status]}`}>
              {statusLabel[status.status]}
            </span>
          )}

          <Button className="rounded-[8px] text-xs" disabled={isFetching} onClick={refetch} size="sm" type="button" variant="glass">
      ...
      {!error && sortedComponents.length === 0 && (
        <div className="rounded-[11px] border border-white/6 bg-white/1.5 px-4 py-4 text-sm text-muted-foreground" role="status">
      ...
      {!error && sortedComponents.length > 0 && (
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {sortedComponents.map(component => (
            <div className="rounded-[11px] border border-white/6 bg-white/1.5 px-3.5 py-3.25" key={component.name}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`size-1.75 shrink-0 rounded-full ${dotTone[component.status]}`} />

                    <p className="text-[13px] font-semibold text-foreground">{component.name}</p>
                  </div>

                  <p className="mt-1.75 line-clamp-2 min-h-7.5 text-[11px] leading-[1.4] text-(--nurt-t3)">
                    {component.message}
                  </p>
                </div>
              ...
              <div className="mt-1.75 flex items-center justify-between gap-3 font-mono text-[10.5px] text-(--nurt-t3)">
              ...
              {component.queues && component.queues.length > 0 && (
                <div className="mt-4 grid gap-2">
                  {component.queues.map(queue => <RabbitQueueRow key={queue.name} queue={queue} />)}
                </div>
              )}
            </div>
          ))}
