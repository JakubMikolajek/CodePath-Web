import type { RepoApiRunnerCollection } from '@workspace/codepath-common/api-explorer'

interface ApiExplorerCollectionsPanelProps {
  collections: RepoApiRunnerCollection[];
  name: string;
  onDelete: (id: number) => void;
  onLoad: (collection: RepoApiRunnerCollection) => void;
  onNameChange: (name: string) => void;
  onSave: () => void;
}

export function ApiExplorerCollectionsPanel({ collections, name, onDelete, onLoad, onNameChange, onSave }: ApiExplorerCollectionsPanelProps) {
  return (
    <div className="space-y-3 rounded-[11px] border border-white/6 bg-white/[0.012] p-3.5">
      <p className="text-xs font-medium uppercase text-muted-foreground">
        Request collections (workspace-shared)
      </p>
      ...
      {collections.length === 0 ? (
        <p className="text-xs text-muted-foreground">
          No saved collections for this repo yet.
        </p>
      ) : (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-muted-foreground">
                  {collection.config.endpointMethod ?? 'UNKNOWN'}{' '}
                  {collection.config.endpointPath ?? ''}
                </p>
              </div>
