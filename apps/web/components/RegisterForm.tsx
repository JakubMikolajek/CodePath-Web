...
export default function RegisterForm({ handleShowRegisterForm }: RegisterFormProps) {
  ...
  const [fallbackError, setFallbackError] = useState<Nullable<string>>(null)
  const [formData, setFormData] = useState<RegisterFormState>({ email: '', login: '', password: '' })
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await dispatch(register({ email: formData.email, login: formData.login, password: formData.password })).unwrap()
      setSuccess(true)
      setTimeout(() => {
      ...
      if (!error) {
        setFallbackError('Registration failed. Check the form and try again.')
      }
    }
  }
