...
export default function LoginForm({ handleShowRegisterForm }: LoginFormProps) {
  ...
  const [formData, setFormData] = useState<LoginFormState>({ identifier: '', password: '' })
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await dispatch(login({ identifier: formData.identifier, password: formData.password })).unwrap()
    } catch (submitError: unknown) {
      console.error('Login request failed', submitError)

      if (!error) {
        setFallbackError('Login failed. Check your credentials and try again.')
      }
    }
  }
