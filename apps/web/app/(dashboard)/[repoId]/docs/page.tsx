  ...
  return (
    <div className="space-y-4.5">
      <PageHeader
        actions={(
          <div className="flex flex-wrap items-center justify-end gap-2">
            <Button className="rounded-[8px] px-2.75 py-1.5 text-[11.5px]" disabled={!canRetryClone || isPipelineActionRunning || isGenerating} onClick={handleRetryClone} type="button" variant="glass">
              <RotateCcw className="size-4" />
              {pipelineAction === 'clone' ? 'Restarting...' : 'Restart clone'}
            </Button>

            <Button className="rounded-[8px] px-2.75 py-1.5 text-[11.5px]" disabled={!canRetryIngest || isPipelineActionRunning || isGenerating} onClick={handleRetryIngest} type="button" variant="glass">
              <RefreshCw className="size-4" />
              {pipelineAction === 'ingest' ? 'Restarting...' : 'Restart ingest'}
            </Button>

            <Button className="rounded-[8px] px-2.75 py-1.5 text-[11.5px]" disabled={!canGenerate || isGenerating || isPipelineActionRunning} onClick={() => handleGenerate('repository')} type="button" variant="glow">
              <Sparkles className="size-4" />
              {generationAction === 'repository' ? 'Starting...' : 'Generate docs'}
            </Button>

            <Button className="rounded-[8px] px-2.75 py-1.5 text-[11.5px]" disabled={!canGenerate || !activeModule || isGenerating || isPipelineActionRunning} onClick={() => handleGenerate('module')} type="button" variant="glass">
              <BookOpen className="size-4" />
              {generationAction === 'module' ? 'Starting...' : 'Regenerate module'}
            </Button>

            <Button className="rounded-[8px] px-2.75 py-1.5 text-[11.5px]" disabled={!canGenerate || !activeModule || !activeSection || isGenerating || isPipelineActionRunning} onClick={() => handleGenerate('section')} type="button" variant="glass">
              <FileText className="size-4" />
              {generationAction === 'section' ? 'Starting...' : 'Regenerate section'}
            </Button>

            <Button className="rounded-[8px] px-2.75 py-1.5 text-[11.5px]" disabled={loading || isPipelineActionRunning || isGenerating} onClick={loadPageState} type="button" variant="glass">
              <RefreshCw className="size-4" />
              Refresh
            </Button>
          </div>
      ...
      <div className="flex min-h-120 gap-4">
        <aside aria-label="Documentation navigation" className="flex w-70 shrink-0 flex-col overflow-hidden rounded-[14px] border border-white/6 bg-white/[0.012]">
              ...
              <div className="space-y-2 border-t border-white/10 pt-4">
                ...
                {activeSections.map(section => {
                  ...
                  return (
                    <button
                      aria-current={isActive ? 'page' : undefined}
                      className={`relative flex w-full items-center justify-between gap-3 rounded-[8px] px-2.5 py-2 text-left text-xs transition ${isActive ? 'bg-primary/15 text-foreground before:absolute before:left-0 before:top-1.75 bbefore:bottom-1.75before:w-0.5 before:rounded before:bg-primary' : 'text-muted-foreground hohover:bg-white/3over:text-foreground'}`}
          ...
          <Card className="min-h-full overflow-hidden rounded-[14px] border-white/6 bg-white/[0.012] py-0">
            <CardContent className="p-0">
              <div aria-live="polite">
                {loading && <p className="border-b border-white/6 px-4.5 py-3.5 text-[12.5px] text-muted-foreground">Loading documentation state...</p>}
                ...
                {!loading && !error && status?.docsStatus === RepoDocsStatus.PENDING && (
                  <p className="border-b border-white/6 px-4.5 py-3.5 text-[12.5px] text-muted-foreground">
