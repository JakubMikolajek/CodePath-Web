...
export class ApiSchemaExtractor {
  ...
  private extractBracedBlock(content: string, startBraceIndex: number): Nullable<string> {
    if (startBraceIndex < 0 || startBraceIndex >= content.length || content[startBraceIndex] !== '{') return null
    ...
    for (let index = startBraceIndex; index < content.length; index += 1) {
      ...
      if (char === '{') {
        ...
        if (depth === 0) return content.slice(startBraceIndex + 1, index)
      }
    }
  ...
  private extractPythonObjectSchemas(content: string): Record<string, RepoOpenApiSchema> {
    ...
    for (const classMatch of content.matchAll(classPattern)) {
      ...
      if (!typeName || !/\b(BaseModel|Serializer|Schema)\b/.test(bases)) continue
      ...
      for (const fieldMatch of body.matchAll(/^[ \t]+([A-Za-z_][A-Za-z0-9_]*)\s*:\s*([^\n=]+)(?:\s*=\s*(.+))?$/gm)) {
        ...
        if (!hasDefault && !/\bOptional\[/i.test(rawType)) required.push(name)
      }
  ...
  private extractTypeScriptObjectSchemas(content: string): Record<string, RepoOpenApiSchema> {
    ...
    for (const match of content.matchAll(objectPattern)) {
      ...
      if (!typeName || /(Controller|Service|Repository|Module)$/.test(typeName)) continue
  ...
  private inferSchemaFromTypeHint(rawType: string): RepoOpenApiSchema {
    ...
    if (!normalized) return { type: RepoOpenApiSchemaType.STRING }
    if (lower.startsWith('array<') || /\[\]$/.test(normalized) || lower.startsWith('list[') || lower.startsWith('sequence[')) return { items: { type: RepoOpenApiSchemaType.STRING }, type: RepoOpenApiSchemaType.ARRAY }
    if (/\b(boolean|bool)\b/i.test(normalized)) return { type: RepoOpenApiSchemaType.BOOLEAN }
    if (/\b(number|float|double|decimal)\b/i.test(normalized)) return { type: RepoOpenApiSchemaType.NUMBER }
    if (/\b(integer|int)\b/i.test(normalized)) return { type: RepoOpenApiSchemaType.INTEGER }
    if (/\b(object|dict|record)\b/i.test(normalized)) return { additionalProperties: true, type: RepoOpenApiSchemaType.OBJECT }
