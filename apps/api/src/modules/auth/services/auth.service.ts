...
export class AuthService {
  ...
  private async findOrCreateExternalUser(identity: {
    ...
    if (!identity.subject) {
      throw new UnauthorizedException('Missing Keycloak subject')
    }

    const normalizedEmail = identity.email?.trim().toLowerCase()

    if (!normalizedEmail) {
      throw new UnauthorizedException('Missing Keycloak email')
    }
    ...
    if (existingUser) {
      if (existingUser.authProvider === 'keycloak' && existingUser.authSubject === identity.subject) {
        return existingUser
      }
  ...
  private async verifyKeycloakJwt(token: string): Promise<KeycloakClaims> {
    ...
    if (claims.iss !== env.keycloakIssuer.replace(/\/+$/, '')) {
      throw new UnauthorizedException('Invalid token issuer')
    }

    if (!this.isExpectedAudience(claims)) {
