...
import { StorageProvider } from '@workspace/codepath-common/ingest'
import { INGEST_CONTRACT_VERSION_V2 } from '@workspace/codepath-common/ingest'
import { IngestMessageType, IngestProducer } from '@workspace/codepath-common/ingest'
...
export function assertValidIngestJobRequestFromWeb(message: IngestJobRequestV2): void {
  ...
  if (message.messageType !== IngestMessageType.JOB_REQUEST) throw new Error(`ingest.messageType must be '${IngestMessageType.JOB_REQUEST}'`)
  if (message.producer !== IngestProducer.WEB_API) throw new Error(`ingest.producer must be '${IngestProducer.WEB_API}'`)
  ...
  if (message.payload.snapshot.provider !== StorageProvider.MINIO) throw new Error(`ingest.payload.snapshot.provider must be '${StorageProvider.MINIO}'`)
