export class GraphNoiseFilter {
  isNodeModulesFilePath(filePath: string) {
    return filePath.toLowerCase().includes('/node_modules/')
  }

  isNodeModuleSpecifier(specifier: string, language: string, fileExt: string) {
    ...
    const isNodeJsLikeSource = (
      lowerLanguage === 'javascript'
      || lowerLanguage === 'typescript'
      || ['.js', '.jsx', '.cjs', '.mjs', '.ts', '.tsx'].includes(lowerExt)
    )

    if (!isNodeJsLikeSource) {
      return false
    }

    if (normalizedSpecifier.includes('/node_modules/')) {
      return true
    }

    if (normalizedSpecifier.startsWith('node:') || normalizedSpecifier.startsWith('npm:')) {
      return true
    }
