...
export interface RepoTopologyImportResolution {
  kind: Nullable<RepoImportResolutionKind>
  resolvedPath: Nullable<string>
}
...
interface CandidateBoundary {
  ...
  moduleId: string
  packageName?: string
  source: CandidateBoundarySource
...
export class RepoTopologyDetector {
  detect(files: Map<string, RepoTopologyFileInput>): RepoTopologyContext {
    ...
    for (const [filePath, file] of files.entries()) {
      ...
      normalizedFiles.set(normalizedPath, { ...file, filePath: normalizedPath })
    }
    ...
    const topologyMode: RepoTopologyMode = workspaceRoots.length > 0 ? 'workspace' : packageManifests.length > 0 ? 'package-json' : 'path'
    ...
    return {
      ...
      resolveImport: (sourceFilePath, specifier, knownFilePaths) => {
        ...
        return { kind: null, resolvedPath: null }
      },
      topologyMode
    }
  }
  ...
  private buildPackageRegistry(packageManifests: PackageManifest[]) {
    return packageManifests.filter(manifest => manifest.name).map<PackageRegistryEntry>(manifest => ({
      dir: manifest.dir,
  ...
  private detectBoundaries(
    ...
    for (const manifest of packageManifests) {
      boundaries.set(manifest.dir, {
        ...
        moduleId: manifest.name ? `package:${manifest.name}` : `package:${this.displayDir(manifest.dir)}`,
        packageName: manifest.name,
        source: manifest.isWorkspaceRoot ? CandidateBoundarySource.WORKSPACE : CandidateBoundarySource.PACKAGE,
    ...
    for (const workspaceRoot of workspaceRoots) {
      for (const candidateDir of candidateDirectories) {
        if (candidateDir === workspaceRoot.dir) continue
        if (!this.isAncestorPath(workspaceRoot.dir, candidateDir)) continue

        const relativeDir = this.relativeToAncestor(workspaceRoot.dir, candidateDir)

        if (!relativeDir) continue
        if (!this.matchesAnyWorkspacePattern(relativeDir, workspaceRoot.patterns)) continue
        if (boundaries.has(candidateDir)) continue

        boundaries.set(candidateDir, {
          dir: candidateDir,
          label: this.displayDir(candidateDir),
          moduleId: `workspace:${this.displayDir(candidateDir)}`,
          source: CandidateBoundarySource.WORKSPACE,
  ...
  private detectWorkspaceRoots(packageManifests: PackageManifest[], files: Map<string, RepoTopologyFileInput>): WorkspaceRoot[] {
    const roots: WorkspaceRoot[] = []
    const workspaceYamlDirs = new Set<string>()

    for (const filePath of files.keys()) {
      if (pathPosix.basename(filePath) === 'pnpm-workspace.yaml') workspaceYamlDirs.add(pathPosix.dirname(filePath))
    }

    for (const manifest of packageManifests) {
      if (manifest.isWorkspaceRoot) roots.push({ dir: manifest.dir, patterns: manifest.workspacePatterns })
    }

    for (const dir of workspaceYamlDirs) {
      if (!roots.some(root => root.dir === dir)) roots.push({ dir, patterns: this.parsePnpmWorkspacePatterns(files.get(pathPosix.join(dir, 'pnpm-workspace.yaml'))?.content ?? '') })
    }

    return roots
  }

  private displayDir(dir: string): string {
    return dir === '.' ? 'root' : dir
  }
  ...
  private exportPatternMatches(pattern: string, requestedSubpath: string): boolean {
    if (!pattern.includes('*')) return pattern === requestedSubpath || pattern === `./${requestedSubpath}`

    const regex = new RegExp(`^${this.escapeRegex(pattern).replace(/\\\*/g, '(.*)')}$`)
    return regex.test(requestedSubpath) || regex.test(`./${requestedSubpath}`)
  }

  private extractExportTargets(exportsField: unknown, requestedSubpath: string): string[] {
    ...
    const visit = (value: unknown) => {
      if (typeof value === 'string') {
        if (value.trim().startsWith('.')) targets.push(value.trim())
      ...
      for (const [key, nestedValue] of Object.entries(value as Record<string, unknown>)) {
        if (key === requestedSubpath || (requestedSubpath !== '.' && key.startsWith('./') && this.exportPatternMatches(key, requestedSubpath))) visit(nestedValue)
      }
    }
    ...
    if (typeof exportsField === 'object') {
      ...
      if (Object.prototype.hasOwnProperty.call(record, requestedSubpath)) visit(record[requestedSubpath])

      for (const [key, value] of Object.entries(record)) {
        if (this.exportPatternMatches(key, requestedSubpath)) visit(value)
      }
    }
  ...
  private extractWorkspacePatterns(content: Record<string, unknown>, files: Map<string, RepoTopologyFileInput>, dir: string): string[] {
    ...
    if (pnpmWorkspace) return this.parsePnpmWorkspacePatterns(pnpmWorkspace.content)

    return []
  }

  private fallbackPathGroup(filePath: string): string {
    ...
    if (first === 'apps' || first === 'packages' || first === 'services' || first === 'libs' || first === 'modules') return second ? `${first}/${second}` : first
    if (first === 'src' || first === 'app' || first === 'lib' || first === 'components' || first === 'features') return second ? `${first}/${second}` : first

    return second ? `${first}/${second}` : first
  }

  private findBoundaryForFile(filePath: string, boundaries: CandidateBoundary[]): Nullable<CandidateBoundary> {
    for (const boundary of boundaries) {
      if (boundary.dir === filePath) return boundary
      if (this.isAncestorPath(boundary.dir, filePath)) return boundary
    }

    return null
  }

  private isAncestorPath(ancestor: string, descendant: string): boolean {
    if (ancestor === '.') return true

    return descendant === ancestor || descendant.startsWith(`${ancestor}/`)
  }

  private isLocalConfigExtends(configPath: string, extendsValue: string): boolean {
    if (!extendsValue.startsWith('.')) return false
  ...
  private matchAliasPattern(pattern: string, specifier: string) {
    if (!pattern.includes('*')) return pattern === specifier ? { capture: '' } : null
  ...
  private matchesAnyWorkspacePattern(relativeDir: string, patterns: string[]) {
    for (const pattern of patterns) {
      if (this.workspacePatternToRegExp(pattern).test(relativeDir)) return true
    }
  ...
  private normalizeCompilerOptions(compilerOptions: unknown) {
    if (!compilerOptions || typeof compilerOptions !== 'object') return { baseUrl: null as Nullable<string>, paths: [] as Array<{ pattern: string, targets: string[] }> }

    const typedOptions = compilerOptions as Record<string, unknown>
    const baseUrl = typeof typedOptions.baseUrl === 'string' && typedOptions.baseUrl.trim().length > 0 ? typedOptions.baseUrl.trim() : null
    ...
    if (rawPaths && typeof rawPaths === 'object' && !Array.isArray(rawPaths)) {
      for (const [pattern, value] of Object.entries(rawPaths as Record<string, unknown>)) {
        ...
        if (pattern.trim().length > 0 && targets.length > 0) paths.push({ pattern: pattern.trim(), targets })
      }
    }
  ...
  private normalizePath(value: string): Nullable<string> {
    const normalized = value.trim().replaceAll('\\', '/').replace(/^\.\/+/, '').replace(/\/{2,}/g, '/')
    return normalized.length > 0 ? normalized : null
  }

  private packageEntryCandidates(packageEntry: PackageRegistryEntry): string[] {
    ...
    if (packageMain) candidates.push(pathPosix.join(packageEntry.dir, packageMain))
  ...
  private parsePackageManifests(files: Map<string, RepoTopologyFileInput>): PackageManifest[] {
    const manifests: PackageManifest[] = []

    for (const [filePath, file] of files.entries()) {
      if (pathPosix.basename(filePath) !== 'package.json') continue
  ...
  private parseTsConfigScopes(files: Map<string, RepoTopologyFileInput>): TsConfigScope[] {
    const scopes: TsConfigScope[] = []

    for (const [filePath, file] of files.entries()) {
      const baseName = pathPosix.basename(filePath)

      if (!baseName.startsWith('tsconfig') && !baseName.startsWith('jsconfig')) continue
  ...
  private readStringManifestField(manifest: Record<string, unknown>, keys: string[]): Nullable<string> {
  ...
  private relativeToAncestor(ancestor: string, descendant: string): Nullable<string> {
    if (!this.isAncestorPath(ancestor, descendant)) return null
  ...
  private resolveAliasImport(
    ...
    if (specifier.startsWith('./') || specifier.startsWith('../') || specifier.startsWith('/')) return { kind: null, resolvedPath: null }

    const orderedScopes = scopes.filter(scope => this.isAncestorPath(scope.dir, sourceFilePath)).sort((left, right) => right.dir.length - left.dir.length)
    ...
    for (const scope of orderedScopes.length > 0 ? orderedScopes : fallbackScopes) {
      for (const alias of scope.paths) {
        ...
        for (const target of alias.targets) {
          ...
          if (candidate) return { kind: 'alias', resolvedPath: candidate }
        }
      }
    }
  ...
  private resolveCandidateFromBase(baseDir: string, relativeCandidate: string, knownFilePaths: Set<string>): Nullable<string> {
  ...
  private resolveCandidates(candidatePaths: string[], knownFilePaths: Set<string>): Nullable<string> {
  ...
  private resolveConfigExtendsPath(configPath: string, extendsValue: string): string {
    const resolved = pathPosix.normalize(pathPosix.join(pathPosix.dirname(configPath), extendsValue))

    if (pathPosix.extname(resolved) !== '.json') return `${resolved}.json`

    return resolved
  }

  private resolveKnownPathCandidate(candidatePath: string, knownFilePaths: Set<string>): Nullable<string> {
    if (knownFilePaths.has(candidatePath)) return candidatePath
    ...
    const extensions = hasKnownExtension ? [''] : DEFAULT_CODE_EXTENSIONS

    for (const extension of extensions) {
      const withExtension = `${candidatePath}${extension}`

      if (knownFilePaths.has(withExtension)) return withExtension

      const indexFile = pathPosix.join(candidatePath, `index${extension}`)

      if (knownFilePaths.has(indexFile)) return indexFile
    }
  ...
  private resolvePackageImport(
    ...
    if (!packageEntry) return { kind: null, resolvedPath: null }

    const subpath = specifier === packageEntry.name ? '' : specifier.slice(packageEntry.name.length + 1)
    ...
    if (resolved) return { kind: 'package', resolvedPath: resolved }

    return { kind: null, resolvedPath: null }
  }

  private resolveRelativeImport(sourceFilePath: string, specifier: string, knownFilePaths: Set<string>): RepoTopologyImportResolution {
    const isRelative = specifier.startsWith('./') || specifier.startsWith('../') || specifier.startsWith('/')

    if (!isRelative) return { kind: null, resolvedPath: null }

    const baseDir = pathPosix.dirname(sourceFilePath)
    const resolvedPath = specifier.startsWith('/') ? pathPosix.normalize(specifier.slice(1)) : pathPosix.normalize(pathPosix.join(baseDir, specifier))

    const resolved = this.resolveKnownPathCandidate(resolvedPath, knownFilePaths)
    if (resolved) return { kind: 'relative', resolvedPath: resolved }
